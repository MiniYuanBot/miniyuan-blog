...
| 时延类型                 | 定义                             | 影响因素          | 类比                       |
| ------------------------ | -------------------------------- | ----------------- | -------------------------- |
| **发送时延（传输时延）** | 数据帧从结点进入传输媒体的时间   | 数据长度/发送速率 | 车辆由收费站进入高速的时间 |
| **传播时延**             | 电磁波在信道中传播一定距离的时间 | 信道长度/传播速率 | 车辆在高速上的行驶时间     |
| **处理时延**             | 主机或路由器处理分组的时间       | 处理复杂度        | 在服务区[^3]检查车况、加油 |
| **排队时延**             | 分组在队列中等待处理的时间       | 网络通信量        | 在收费站前排队等待的时间   |
...
**高速链路（高带宽链路）**：提高的是**数据发送速率**（减小发送时延），而非比特传播速率，类似更多的车道，上高速更快。
